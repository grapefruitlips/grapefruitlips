...
function test_input($data) {
  ...
  return $data;
	...
	if(empty($username)||empty($email)||empty($text)) {
		header("Location: /index.php?err=baddata");
		exit;
	}
...
?>
<div class="collapse <? if(isset($_GET['err'])) { ?>show <?} ?><? if(isset($_GET['edit']) && $logged_in==1) {?>show <? } ?>mxw mrgb" id="collapseExample">
  <div class="card card-body grbg">
<form method="post">
  <? if($edit==1){?>
 ...
 <div class="form-group">
  ...
  <? } ?>
   <? if(isset($_GET['err'])) { ?>
  <div class="form-group">
  <small class="form-text text-muted"><span class="text-danger">Не заполнены все поля: имя пользователя, email и описание задачи.</span></small>
    </div>
  <? } ?> 
...
<div class="mxw">
<? if(isset($_GET['added'])) { ?><span class="oknow text-success">Задача была добавлена</span><?} ?>
<nav class="txar">
  <ul class="pagination">
